...
export default function LaporanOperasiPage() {
  ...
  const handleDownloadPdf = () => {
    ...
    // Collect all data from downloadCategories (based on selectedCategory filter)
    ...
    downloadPdfMulti({
      ...
      period: selectedDate ? `Tanggal: ${toIndonesianDate(selectedDate + 'T00:00:00')}` : 'Semua Tanggal',
      sections: allSections,
      filename: `Laporan_Operasi_${selectedDate || "all"}`,
    });
  };
  ...
  const handleDownloadExcel = () => {
    ...
    const filename = `Laporan_Operasi_${selectedDate || "all"}.xlsx`;
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Lap. Operasi Harian</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-4 sm:px-6 py-3 space-y-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-6 gap-2">
          {/* Date filter - full width on mobile, 1 column on desktop */}
          <input
            type="date"
            value={selectedDate}
