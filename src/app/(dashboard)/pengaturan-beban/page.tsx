...
export default function PengaturanBebanPage() {
  ...
  const handleSyncToEquipmentLogs = async () => {
    if (!syncP2BId || !syncP2BData) return;
    ...
    try {
      ...
      if (!p2b.unit_pindah || p2b.unit_pindah.trim() === "") {
        alert("Data ini tidak memiliki unit_pindah.");
        setSyncing(false);
        return;
      }
      ...
      for (const unitName of unitNames) {
        ...
        const payload = {
          ...
          shift: p2b.shift || lastLogAny?.shift || "Dayshift",
          update_beban_pln: p2b.update_beban_pln || null,
          update_beban_btg: p2b.update_beban_btg || null,
    ...
    <div className="space-y-6">
      ...
      {showSyncConfirm && syncP2BData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm" onClick={() => { setShowSyncConfirm(false); setSyncP2BId(null); setSyncP2BData(null); }}>
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-base font-semibold text-gray-900 mb-2">Sinkronisasi Data?</h3>
            <p className="text-sm text-gray-500 mb-2">Equipment: <span className="font-semibold text-gray-900">{syncP2BData.unit_pindah}</span></p>
            <p className="text-sm text-gray-500 mb-6">Apakah Anda akan melakukan sinkron data terhadap equipment ini?</p>
            <div className="flex items-center justify-end gap-3">
              <button onClick={() => { setShowSyncConfirm(false); setSyncP2BId(null); setSyncP2BData(null); }} className="px-4 py-2.5 text-sm font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors">Tidak</button>
              <button onClick={handleSyncToEquipmentLogs} disabled={syncing} className="px-4 py-2.5 text-sm font-semibold text-white bg-green-600 hover:bg-green-700 rounded-xl transition-colors flex items-center gap-2 disabled:opacity-50">
